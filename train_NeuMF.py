...
from src.models import NeuralMF, Bert4Rec, NeuMF
...
def main(config_path):
    ...
    data_manager = DataManager(cfg.model.type, cfg.dataset.name, cfg.training.batch_size, cfg.model.params.get("max_sequence_length", 50))
    ...
    match cfg.model.type:
        ...
        case "bert":        
            os.makedirs(cfg.saving.save_dir, exist_ok=True)
            train_losses, hit_list, ndcg_list, eval_at = trainer.train_n_steps_bert(
                data_manager.train_loader, data_manager.valid_loader, 
                accumulation_steps=cfg.training.accumulation_steps, 
                validation_interval=cfg.evaluation.interval, 
                max_steps=cfg.training.accumulation_steps*cfg.training.update_steps, 
                save_path=f"{cfg.saving.save_dir}/{cfg.saving.filename}_{cfg.dataset.name}_{cfg.seed}", 
                cfg=cfg)
